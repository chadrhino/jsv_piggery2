    ...
    <div class="w3-row">
      ...
      <form method="POST" class="table-responsive">
        <table class="table table-hover table-striped" id="table">
          ...
          <tbody>
            <?php
            $all_pig = $db->query("SELECT p.*,b.name AS breed_name, c.name AS class_name, v.name AS vita_name, f.name AS feed_name  FROM pigs p LEFT JOIN breed b ON p.breed_id = b.id LEFT JOIN vitamins v ON p.vitamins_id = v.id LEFT JOIN classification c ON p.classification_id = c.id LEFT JOIN feed f ON p.feed_id = f.id WHERE p.status = 1 AND p.cage_num = 3 AND p.type IS NULL ORDER BY p.id DESC ");
...
<script>
  ...
  function showCage(x,gender, cage) {
    Swal.fire({
      ...
      /* Read more about isConfirmed, isDenied below */
      if (result.isConfirmed) {
        window.location.href = "manage-cage3.php?id=" + x + "&cage=" + cage;
      }
    });
  }
